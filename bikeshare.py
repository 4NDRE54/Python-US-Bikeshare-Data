...
def time_stats(df):
    ...
    df['hour'] = df['Start Time'].dt.day_name
    popular_dow = df['month'].mode()[0]
...
def station_stats(df):
    ...
    print ('The most commonly used end stationis ' , most_end)

    # TO DO: display most frequent combination of start station and end station trip
    comb_station = df.groupby(['Start Station','End Station']).size().nlargest(1)
    print ('The most most frequent combination of start station and end station is ' , comb_station)
...
def user_stats(df):
    ...
    print ('These are the individual user types: \n' , user_type)
    # TO DO: Display counts of gender
    if 'Gender Taype' in df:
        gender_type = df['Gender'].value_counts()
        print ('These are the individual user types: \n' , gender_type)
    else:
        print ('There is no Gender Type data')
    

    # TO DO: Display earliest, most recent, and most common year of birth
    if 'Gender Taype' in df:
        earliest = df['Birth Year'].value_counts().idxmin()
        recent = df['Birth Year'].max()
        common = df['Birth Year'].value_counts().idxmax()
        print ('These are the earliest, most recent, and most common year of birth: \n' , gender_type)
    else:
...
def raw_data(df):
    #5 lines of raw data; more on request
    row = 0
    while (input ('Would you like to see 5 lines of raw data? Enter yes or no.\n').lower() != 'no'):
        row += 5
        print (df.head(row))
